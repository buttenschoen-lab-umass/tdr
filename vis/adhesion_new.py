...
def get_max_values(dfs):
    return np.max(dfs[0].values)


def create_animation(datafile, L, basename):
    df = load_datafile(datafile).transpose()
    # TEMP HACK
    dfs={}
    dfs[0] = df
    ymax = np.round(1.2 * get_max_values(dfs), decimals=1)
    fig = plt.figure()
